...
public:
  ...
  void turnOn(int Col, int Row, int delayTime = 2);

  // Turn on entire y-Row
  void OnRow(int Row, int delayTime = 2);

  // Turn on entire x-Col
  void OnCol(int Col, int delayTime = 2);

  // Turn on x-Col by given array, shiftable by +/- n
  void customCol(int array[], int Col, int shift = 0, int delayTime = 2);

  // Turn on y-Row by given array, shiftable by +/- n
  void customRow(int array[], int Row, int shift = 0, int delayTime = 2);

  // Check all possible LED
  void Test(int delayTime = 100);

  // Display custom symbol
  void Symbol(int UserMatrix[RowSize][ColSize], unsigned long showTime = 1000, bool clear = 0);
